...
const List: React.FC = () => {
  ...
  console.log("ssdasdadsadsadasdasdas", maxPrice);
  // Filter and sort the domain data
  const filteredDomainData = domainData
    .filter((domain: Domain) => {
      const symbolCount = domain.name.length;
      const priceLari = domain.priceLari;
      const isPriceValid =
        (!minPrice || priceLari >= parseFloat(minPrice)) &&
        (!maxPrice || priceLari <= parseFloat(maxPrice));
      const isSymbolValid =
        (!symbolMin || domain.name.includes(symbolMin)) &&
        (!symbolMax || domain.name.includes(symbolMax));
      ...
      const isDomainEndpointValid =
        selectedDomzones.length === 0 ||
        selectedDomzones.includes(`.${domainEndpoint}`);
      const isCharPrefixValid =
        !charPrefix || domain.name.startsWith(charPrefix);
      const isSearchMatch = domain.name
        .toLowerCase()
